...
- **A trend beside the figure.** Each quote row can carry an intraday series (the payload's `SPARKS`), drawn as a small red/green sparkline with a dashed rule at the value it started from, its high and low, and the window it covers — indexes and stocks from the open to the moment the brief is compiled, crypto over the last 24 hours. The email cannot show a drawing (the mail path strips images and SVG), so it carries the same shape as block characters.
- **The index column follows the hour**: the morning brief heads it `open`, the evening update `close`. The name and its figure share one column — `S&P 500` over `6,412.30 (9:30 AM EST)` — and the date appears beside a figure only when it was taken on another day, because the masthead already dates the brief.
